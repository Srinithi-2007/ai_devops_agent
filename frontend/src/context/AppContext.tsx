...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const analyzeIncident = useCallback(async () => {
    setAnalyzing(true)
    addNotification('info', 'Sending incident to Amazon Bedrock for analysis...')
    await new Promise(r => setTimeout(r, 2800))
    const newId = `INC-${2848 + Math.floor(Math.random() * 10)}`
    const newIncident: Incident = {
      id: newId,
      service: ['inventory-service', 'checkout-service', 'recommendation-engine'][Math.floor(Math.random() * 3)],
      severity: 'high',
      error: 'SocketTimeoutException: Read timed out after 5000ms connecting to downstream service',
      summary: 'Downstream service read timeout causing cascading failures in dependent services',
      rootCause: 'Slow database query (full table scan on orders table missing index on created_at column)',
      recommendation: 'Add composite index on (service_id, created_at), enable query timeout of 2s, implement retry with exponential backoff',
      confidence: 88,
      status: 'resolved',
      createdAt: new Date().toISOString(),
      similarIncidents: [
        { id: 'INC-2200', service: 'checkout-service', error: 'Read timeout', similarity: 89, resolvedAt: '2026-08-01T10:00:00Z' },
      ],
    }
    setIncidents(prev => [newIncident, ...prev])
    setAnalyzing(false)
    addNotification('success', `Analysis complete — ${newId} stored in CockroachDB memory with 88% confidence`)
  }, [addNotification])
